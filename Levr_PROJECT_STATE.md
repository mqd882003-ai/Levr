...
> Last updated: **2026-08-30** (later same-day session, continued) — **View-count question
> resolved: Dave decided all four calendar views ship** (Week done, Day/Month/Calendar to
> follow) — the "ask first" blocker from the previous update is lifted. **Phase 2 item 3 —
> Day view** shipped: vertical agenda, windows+deadlines merged and sorted
> chronologically together (a deliberate choice, documented in `CalendarClient`'s header
> comment — differs from Week's still-unsorted deadlines-before-windows listing, which
> was left as-is, not "fixed," since only Day was asked for). Functional Day/Week toggle
> (Day was previously a disabled stub); prev/next arrows and the Today-jump chip are now
> view-aware; tapping a deadline card opens the real `EntrySheet`. New `.day-card` family
> in `globals.css` (full-width cards, distinct from Week's compact `.mini-block` rows, per
> spec). Committed `303b80a` → GitHub `d90758b`, confirmed live two ways: bundle-grepped
> (`className:\`day-card protected...\``, `"Previous day"`) AND visually confirmed
> working on production (levr-six.vercel.app) itself, not just the served JS. **Pre-item
> sanity check** (asked for before starting): manually-created entries' synthesized
> `explicit_deadline` label (e.g. "Aug 30, 2026, 11:34 AM") can never actually render
> anywhere — both surfaces that show that field in quotes (the undated strip, and
> `DateAssignSheet`'s "Originally said") are gated on `!deadlineAt`, and manual creates
> always carry a real `deadlineAt` by construction. No fix needed; confirmed by grepping
> every read site of `explicitDeadline`/`explicit_deadline`, not just reasoning about it.
> Month + Calendar (hour-grid) views are next (items 4–5), each its own stop-for-review
> session per the usual rule — not started yet.
>
