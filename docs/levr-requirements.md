...
3. System silently commits its best guess to the database — it does NOT block the user with a multi-question form.
   > **Deliberate exception (2026-08-27, Dave-approved):** immediately after a capture is classified on Home, the app may ask up to **3** quick tap-to-answer questions before navigating to Board — only for chunks with no resolvable business, or for newly mentioned names not yet in Team. This is a conscious carve-out from rule 3, not a bug to fix back: the entry is already committed to the DB before any question appears (rule 3's never-lose-a-thought guarantee is untouched), every question is skippable, and anything skipped or beyond the cap falls through to the existing needs-a-look/Review-with-me path exactly as before. Do not "restore" silent capture by removing this without Dave's say-so.
